...
class Imslp:
    ...
    def get_composers(self) -> Dict[str, Url]:
        ...
        doc = BeautifulSoup(self.browser.navigate(url).text, features='html.parser')
    ...
    def get_compositions(self, url: Url) -> Dict[str, Url]:
        ...
        doc = BeautifulSoup(self.browser.navigate(url.url).text, features='html.parser')
    ...
    def get_composition(self, url: Url) -> Generator[Tuple[str, int, Url], None, None]:
        ...
        doc = BeautifulSoup(self.browser.navigate(url.url).text, features='html.parser')
        div: Tag
        for div in doc.select('div#wpscore_tabs > div'):
            for div in div.select('div.we > div[id]'):
                if not div['id'].startswith('IMSLP'):
                    continue
                a = div.select_one('span.mh555 > a[title]')
                if a is None:
                    continue
                file_type = a.text.lower()
                if file_type not in ('pdf',
                                     'mscz'):  # Music score: https://imslp.org/wiki/Herz_und_Mund_und_Tat_und_Leben,_BWV_147_(Bach,_Johann_Sebastian)
                    if file_type not in ('zip',):
                        continue
                    continue
                a = div.select_one('a')
                dl_id = div['id']
                title = div.select_one('span[title]').text.strip()
